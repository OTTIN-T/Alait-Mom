<template>
  <div class="flex flex-col mx-auto">
    <UButton :icon="btnProperty.icon" :color="btnProperty.color" variant="solid" :label="btnProperty.label" size="xl"
      :disabled="btnProperty.disabled" @click="getInstallPrompt" />
    <UPopover v-if="isFirefox || isIOS" class="mx-auto mt-2" :popper="{ arrow: true }">
      <UButton color="orange" :label="labelPopover" icon="i-heroicons-exclamation-triangle-20-solid" size="2xs" />
      <template #panel>
        <p v-if="isFirefox" class="text-sm text-center">
          Sur Firefox vous avez besoin de cet
          <NuxtLink to="https://addons.mozilla.org/fr/firefox/addon/pwas-for-firefox/" class="text-primary">
            addon
